...
Regenerate the real data contract by executing notebooks 04, 06, and 07, plus `scripts/_gen_anomaly_model.py` for the browser-inference artifact. `python -m weather_forecast.dashboard_export` does **not** regenerate it: its CLI emits only `data_status="sample"` and defaults its output directory to `web/public/data`, so running it in place overwrites the committed real contract with synthetic data.
